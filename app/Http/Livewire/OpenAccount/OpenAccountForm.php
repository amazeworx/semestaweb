...
class OpenAccountForm extends Component
{
  ...
  public function submit($currentStep)
  {
    ...
    if ($currentStep === 1) {
      ...
      if ($this->uploadKtp) {
        $this->uploadKtp->storeAs('files/' . $stored_data->id, 'KTP.' . $this->uploadKtp->extension());
      }

      if ($this->uploadPassport) {
        $this->uploadPassport->storeAs('files/' . $stored_data->id, 'Passport.' . $this->uploadPassport->extension());
      }

      if ($this->uploadNpwp) {
        $this->uploadNpwp->storeAs('files/' . $stored_data->id, 'NPWP.' . $this->uploadNpwp->extension());
      }
      ...
      if ($this->uploadSignature) {
        ...
        Storage::put('files/' . $stored_data->id . '/TandaTangan.png', $decoded_image);
      }
