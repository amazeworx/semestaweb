  ...
  <div class="relative">
    <div class="flex justify-between items-center mb-3">
      <div class="flex-grow h-10 flex items-center">
        ...
        <div class="w-96 flex rounded shadow">
          <div class="relative flex-grow focus-within:z-10">
            ...
            <input wire:model.debounce.500ms="search"
              class="form-input block bg-gray-50 focus:bg-white w-full rounded-md py-2 pr-3 pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5"
      ...
      <div class="flex items-center space-x-3">
        ...
        <div x-data="{ init() {
          ...
          <button wire:click="export"
            class="flex items-center space-x-2 px-3 shadow rounded border border-gray-300 bg-gray-50 text-green-500 text-xs leading-4 font-medium uppercase tracking-wide transition hover:bg-white hover:shadow-md focus:outline-none"><span>Export</span>
    ...
    <div class="rounded shadow bg-white max-w-screen">
      <div class="overflow-x-scroll @unless($this->hidePagination) rounded-b-none @endif">
        <div class="table align-middle min-w-full">
          ...
          <div class="table-row divide-x divide-gray-200">
            ...
            <div
              class="relative h-12 w-48 overflow-hidden align-top px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex items-center focus:outline-none">
              <div
          ...
          <div class="table-row divide-x divide-blue-200 bg-blue-100">
            ...
            @elseif($column['type'] === 'checkbox')
            <div
              class="w-32 overflow-hidden align-top px-6 py-5 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
              <div>SELECT ALL</div>
              <div>
                <input type="checkbox" wire:click="toggleSelectAll" @if(count($selected)===$this->results->total())
                checked @endif class="form-checkbox mt-1 h-4 w-4 text-blue-600 transition duration-150 ease-in-out" />
              </div>
            </div>
            @else
          ...
          <div
            class="table-row divide-x divide-gray-100 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50') }}">
            ...
            @else
            <div
              class="px-6 py-4 whitespace-nowrap text-sm leading-5 text-gray-900 table-cell align-top @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif">
              {!! $result->{$column['name']} !!}
            </div>
            @endif
            @endforeach
      ...
      <div class="max-w-screen border-b border-gray-200 bg-white">
        <div class="py-3 px-4 sm:flex items-center justify-between">
